...
export function PropertyManagementForm({ source = 'gestion-locative' }: PropertyManagementFormProps) {
  ...
  const [nouvelleConstruction, setNouvelleConstruction] = useState(false);
...
- Statut: ${occupe ? 'Occupé' : 'Non occupé'}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-prenom">
            Prénom *
          </label>
          <input
            id="pm-prenom"
        ...
        <div>
          <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-nom">
            Nom *
          </label>
          <input
            id="pm-nom"
      ...
      <div>
        <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-contact">
          Contact *
        </label>
      ...
      {/* Type de bien */}
      <div>
        <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-type">
          Type de bien *
        </label>
        <input
          id="pm-type"
          ...
          required
          className={inputClass}
          placeholder="Ex: Villa, Terrain, Célibatérium, Cour unique, Mini villa, Immeuble R+2..."
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-ville">
            Ville *
          </label>
          <input
            id="pm-ville"
        ...
        <div>
          <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-quartier">
            Quartier *
          </label>
          <input
            id="pm-quartier"
      ...
      <div className="space-y-2">
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            type="checkbox"
            checked={occupe}
            onChange={(e) => setOccupe(e.target.checked)}
        ...
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            type="checkbox"
            checked={nouvelleConstruction}
            onChange={(e) => setNouvelleConstruction(e.target.checked)}
            className="w-4 h-4 rounded border-white/15 bg-[var(--color-anthracite)] text-[var(--color-gold)] focus:ring-[var(--color-gold)] focus:ring-offset-0"
          />
          <span className="text-sm text-white group-hover:text-[var(--color-gold)] transition-colors">
      ...
      {/* Message additionnel */}
      <div>
        <label className="text-xs text-[var(--color-white-muted)] block mb-1" htmlFor="pm-message">
